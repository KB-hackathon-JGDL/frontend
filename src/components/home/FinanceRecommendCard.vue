...
const showListProducts = computed(
  () => !!store.persona && store.recommendedProducts && store.recommendedProducts.length > 0
)
...
type CardItem = { title: string; benefit: string }
const cardList = ref<CardItem[]>([
  { title: 'KB국민 톡톡카드',   benefit: '편의점/배달 10% 적립' },
  { title: '신한 Deep Dream', benefit: '생활영역 0.7~5% 적립' },
  { title: '우리카드 오늘',     benefit: '교통/통신 할인' },
])
</script>
...
<template>
  <section class="relative z-[10] -mt-[45px] ml-5">
    <div
      class="w-[430px] h-[567px] mx-auto -translate-x-5 mt-10
             rounded-t-[30px] bg-[#ECF0F3] p-4 pt-10 ring-1 ring-black/5
             shadow-[0_12px_24px_rgba(0,0,0,0.5)]">

      <div class="flex items-center justify-between mb-5 pr-3">
        <h2 class="text-[16px] font-semibold text-gray-800 ml-3">금융상품 추천</h2>
        <button
      ...
      <div class="relative mt-2 px-2">
        ...
        <div class="absolute mt-10 -mb-5 left-0 right-0 bottom-0 h-px bg-gray-200"></div>
        <div
        ...
        <template v-if="showListProducts">
          <div class="grid gap-6 mt-12">
            <div
              ...
              :key="p.title"
              class="w-[350px] h-[80px] mt-[5px] text-left rounded-[40px] bg-white p-4 ml-10 mb-[10px]
                     ring-1 ring-gray-100 shadow-[0_6px_16px_rgba(0,0,0,0.06)]
                     flex items-center justify-between">
              <div class="ml-2 flex items-center gap-5">
                <div class="w-[40px] h-[40px] rounded-full bg-[#FFFFFF] grid place-items-center overflow-hidden">
                  <img :src="kb" alt="" class="w-[35px] h-[35px] object-contain" />
                </div>
                <div>
                  <p class="text-[15px] font-medium text-gray-800">{{ p.title }}</p>
...
>
  <ChevronRight :size="25" class="text-white" />
        ...
        <template v-else>
          <button
            class="w-[350px] h-[120px] text-left rounded-[40px] bg-white p-4 mr-10 mt-10
                   ring-1 ring-gray-100 shadow-[0_6px_16px_rgba(0,0,0,0.06)]
                   flex items-center justify-between"
            @click="startSurvey"
          >
            <div class="ml-8">
              <p class="text-[14px] text-gray-400 mt-0 mb-5">아직 설문을 진행하지 않으셨어요!</p>
              <p class="text-[15px] font-medium text-gray-800">
                설문조사를 진행하고<br />나에게 맞는 금융상품을 알아봐요!
              </p>
            </div>
            <span class="w-14 h-14 rounded-full bg-[#FFC727] grid place-items-center">
        ...
        <div class="grid gap-6 mt-6 mt-[30px]">
          <div
            ...
            <div class="ml-2 flex items-center gap-5">
              ...
              <div>
                <p class="text-[15px] font-medium text-gray-800">{{ c.title }}</p>
