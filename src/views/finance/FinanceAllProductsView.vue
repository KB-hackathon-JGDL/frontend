...
type Product = { id: string; bank: string; title: string; rate?: Rate }

const bankLogoMap: Record<string, string | null> = {
...
const RATE_OVERRIDES: Record<string, Rate> = {
}
...
const list = computed<Product[]>(() => {
  if (props.products?.length) {
    return props.products.map(p => ({ ...p, rate: p.rate ?? RATE_OVERRIDES[p.id] }))
  }
  return [
...
<template>
  <div class="mx-auto w-[430px] min-h-[932px] bg-[#F4F6FE]">
    ...
    <main class="p-4 bg-[#ffffff] h-[900px]">
<div class="relative mt-4 px-2 mb-[10px]">
  ...
  ></div>
      ...
      <div v-for="p in list" :key="p.id"
           class="h-[90px] w-[370px] mx-auto mt-[30px] mb-[30px] flex items-center justify-between bg-[#F0F6FF] rounded-[16px] shadow-sm px-4 py-3">
        <div class="flex items-center gap-3">
          <div class="w-[50px] h-[50px] grid place-items-center rounded-full overflow-hidden">
            <img v-if="bankLogoMap[p.bank]" :src="bankLogoMap[p.bank]!" alt="" class="w-[35px] h-[35px] object-contain" />
            <span v-else class="text-[12px] text-gray-500">{{ p.bank }}</span>
          </div>
          <div>
        ...
        <button class="w-10 h-10 grid place-items-center" @click="toggleFav(p.id)">
